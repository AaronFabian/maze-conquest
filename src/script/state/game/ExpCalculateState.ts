...
export class ExpCalculateState extends BaseState {
	...
	constructor(public battleState: BattleState, public enemies: Party, public heroes: Party) {
		...
		for (let i = 0; i < this.enemies.length; i++) {
			...
			if (this.totalEnemyDefeated.has(enemy.name)) {
				...
				this.battleExpReward += enemy.exp;
			} else {
	...
	private tweenHeroExpProgressBar() {
		...
		for (let i = 0; i < this.progressBars.length; i++) {
			...
			// Copy the reference so we don't lose the real exp reward
			let battleExpReward = this.battleExpReward;
			let tweenFirstRef = null;
			let tweenRef = null;

			// Loop until all experience is allocated
			while (battleExpReward > 0) {
				// 01 Get the difference
				const remainingHeroExpToLevelUp = hero.expToLevel - hero.currentExp;

				if (remainingHeroExpToLevelUp - battleExpReward > 0) {
					// 02 Tween the current state
					// This block add experience without level up
					const ref = new Tween(progressBar).to({ value: progressBar.value + battleExpReward }, 750);
					hero.currentExp += battleExpReward;
					...
					// break;
					battleExpReward -= remainingHeroExpToLevelUp;
				} else {
	...
	override render() {
		...
		if (this.secondPress) {
			...
			ctx.fillText(`Total exp ${this.battleExpReward}`, canvas.width / 2 - 180 + 3, canvas.height / 2 - 240 / 2 + 278);
	...
	override exit() {
		...
		// Reset the animation
		this.battleState.heroParty.party.forEach(hero => hero.changeState('idle'));
	}
}
